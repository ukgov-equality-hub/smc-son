...
input_file = "2026-01-01-in11-full-dataset.csv"
...
all_ethnicity_sex_table = data_for_section_filtered %>%
  ...
  select(ethnicity, primary_split_value, time_period, value)
...
pivot_table = pivot_table__create(
  ...
  pivot_cells_column_name = "value",
  pivot_table_name = "Ethnicity",
  pivot_table_rows_order_values = ethnicity_order_longer_2,
  pivot_table_columns_2_order_values = boys_girls_order_w_total,
  pivot_table_column_names_suffix = " (%)"
)
