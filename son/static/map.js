...
class Choropleth {
    ...
    render(filteredData) {
        ...
        function ready(eudata, geodata, data) {
            ...
            // EU background
            if (self.background) {
                self.bgProjection = d3.geoMercator().fitSize(
                    [self.width, self.height],
                    topoFeatures(eudata)
                )
                self.bg = d3.geoPath(self.bgProjection)
            }
            ...
            if (self.legendDiv != '') {
                if (dataFormat == 'sequential') {
                    ...
                    d3.select(`#${self.legendDiv}`)
                        .append('svg:svg')
                            .attr('width', labelLen + 50)
                            .attr('height', (legends.length * 20) + ((legends.length - 1) * 10))
                            .attr('viewBox', `0 0 ${labelLen + 50} ${(legends.length * 20) + ((legends.length - 1) * 10)}`)
                            .attr('perserveAspectRatio', 'xMinYMid')
                            .attr('class', 'legend')
                        .append('g')
                        .selectAll('g')
        ...
        function getBounds(data, boundary, width = self.width, height = self.height, format = geoFormat) {
            ...
            if (!boundary) {
                ...
                bounds = boundary.bounds(data)
            }

            const scale = 1 / Math.max((bounds[1][0] - bounds[0][0]) / width, (bounds[1][1] - bounds[0][1]) / height),
                transform = [(width - scale * (bounds[1][0] + bounds[0][0])) / 2, (height - scale * (bounds[1][1] + bounds[0][1])) / 2]
            return { bounds, scale, transform }
        }

        function getFeatures(data, key, format = geoFormat) {
            data = format == 'topo' ? data.objects[Object.keys(data.objects)[0]] : data
            if (key && data[key]) data = data[key]
            return data
        }

        function topoFeatures(data) {
